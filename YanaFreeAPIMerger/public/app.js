...
optionButtons.forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    input.placeholder = {
      ...
      general: 'Ask anything fragrance-related…',
    }[selectedIntent];
    input.focus();
  });
});
...
async function runChat(message) {
  ...
  const res = await fetch('/api/chat', {
    ...
    body: JSON.stringify({ message, intent: selectedIntent }),
  });
...
fetch('/api/config').then((r) => r.json()).then((cfg) => {
  if (!cfg.configured) {
    ...
    addMessage(`Council of ${cfg.agentCount} agents is ready. Pick an option below or just type your question.`, 'bot');
  }
}).catch(() => {});
