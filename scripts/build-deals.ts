 ...
 * bolted on beside it.
 */
// A run of this script happens on its own ~6-hourly cadence (see this file's
// own header), independent of the hourly harvest, so "now" is captured once,
// here, rather than one Date.now() per offer drifting across the loop below.
const now = new Date();
