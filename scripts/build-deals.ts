/**
 * Snapshots "Today's Deals" into a file of its own.
 *
 *   npm run deals:build
 *
 * Deals get their own file rather than being computed live on every build the
 * way they used to be, and demo/data.ts resolves it into full Deal records.
 * The original reason was cadence: a reader browsing the deals page for ten
 * minutes should not watch the list quietly reshuffle mid-visit because an
 * unrelated tick landed, so this ran on a 5-hour age gate of its own while
 * every other generated view rebuilt with the harvest.
 *
 * That gate is gone as of 2026-09-01, and the file stays. Since
 * wasPriceCredibility.ts's test zero (2026-08-26) a deal's `wasPrice` can be
 * `CatalogueEntry.houseCeiling`, which lives in demo/catalogue.generated.ts —
 * so this snapshot and that catalogue became one consistent pair, and a pair
 * regenerated on two different cadences drifts into contradiction. It did,
 * twice on 2026-09-01, the second time wedging the branch on
 * tests/dealsBrandDirect.test.ts; see catalogue-daily.yml's "Rebuild the app
 * from harvested prices" for the incident and the numbers. This now runs
 * straight after `npm run catalogue:demo`, every time the catalogue is
 * rebuilt — so the list moves on the three-hourly cron rather than every six
 * hours, which is still nothing like the per-tick churn the paragraph above
 * was written against.
 *
...
import { isStaleFetch } from '../src/services/priceService.js';
