 ...
 * this one in the file — see there for why `sizeMl` reads six of them as a
 * real, confirmed number (settled by a field outside the title) and Red
 * Velvet alone as `null`. isFragrance keeps all seven either way: for Red
 * Velvet, the size rule inside isFragrance has to tell that null apart from
 * the null a title with no size at all also produces, or loosening it to
 * admit these seven would just as easily admit "Fragrance-free baby nappy
 * cream", which is the exact regression NOT_A_FRAGRANCE and this size gate
 * together exist to prevent — see this describe block's second case for
 * that.
 */
 ...
 * Velvet and Club De Nuit Woman perfume oil, and Avon's Full Speed). For one
 * of them — Red Velvet — the ambiguity is still real: title text alone
 * cannot settle it, and neither can any other field this project's tools can
 * reach (see fragranceId.ts's own note beside SIZE_CONFLICT_RESOLVED for
 * everything checked and why each angle failed).
 *
 * `sizeMl` used to return the first number for all seven anyway — a
 * confident size that was right five times out of seven and wrong by one
 * size step (110ml read as 100ml, or the reverse) on the Hamidi pair a
 * first pass hadn't yet resolved, never an order of magnitude off the way
 * the pre-2797294 Al Haramain bug was, but still a number stated with no
 * more basis than a coin flip on those two. It now returns `null` for Red
 * Velvet alone, and `sizeConflict` is what lets a caller — isFragrance
 * below, in particular — tell this apart from a title that names no size at
 * all: see that function's own comment for why the difference decides
 * whether the listing stays in the catalogue. Every downstream consumer of a
 * built product's `sizeMl` (productMatch.ts's matchKey,
 * wasPriceCredibility.ts's CredibilityOffer, demo/volumeBands.ts,
 * demo/listSort.ts, demo/app.ts's facet and tile rendering) treats a null
 * size as "cannot compare", never as a definite number to sort, key or print
 * — see each file's own tests for the specific behaviour.
 *
 * The other six of the seven are the describe block just below this one:
 * unlike Red Velvet, a field outside the title settles each of them, so
 * `sizeMl` reads a real number while `sizeConflict` still reports the title
 * itself as stating two disagreeing sizes — see SIZE_CONFLICT_RESOLVED's own
 * comment in fragranceId.ts for the evidence.
 */
...
/**
 * sizeMl: six of the seven SIZE_CONFLICT_RE titles resolved by a field
 * outside the title, rather than left null — see SIZE_CONFLICT_RESOLVED's
 * own comment in fragranceId.ts for the raw-data evidence. Two (Club De Nuit
 * Woman's perfume oil, Full Speed) by armaf.uk's and avon.uk.com's own
 * `description` fields, plus Full Speed's price agreeing with its other
 * confirmed-100ml lines; four more (Armaf's Hamidi Maison Luxe lines) by
 * hamidi.us — Hamidi's own separate manufacturer domain, checked directly by
 * WebSearch and WebFetch 2026-09-01 after armaf.uk's own description turned
 * out to be wrong for two of the four. `sizeConflict` still reports every
 * one of these titles as conflicting — that fact is about what the title
 * itself states, unchanged by a different field settling it — which is
 * exactly why sizeMl needed the lookup rather than sizeConflict needing an
 * exception.
 */
describe('sizeMl: six of the seven conflicting titles resolved by a field outside the title', () => {
  it.each([
  ...
  ])('reads the confirmed size, not null: %s', (title, expected) => {
    expect(sizeMl(title)).toBe(expected);
    expect(sizeConflict(title)).toBe(true);
  });
});
